      ...
      <form @submit.prevent="onSubmit">
        <div class="input" :class="{invalid: $v.email.$error}">
          ...
          <input
                  ...
                  @blur="$v.email.$touch()" 
                  v-model="email">
                  <!-- @blur runs when input goes out of focus, @input runs when input comes into focus -->
        ...
        <div class="input" :class="{invalid: $v.password.$error}">
          ...
          <input
                  ...
                  v-model="password">
          <p v-if="!$v.password.minLength">Password must be at least {{ $v.password.$params.minLen.min }}</p>
        </div>
        <div class="input" :class="{invalid: $v.confirmPassword.$error}">
          ...
          <p v-if="!$v.confirmPassword.sameAs">Must be same as password.</p>
        </div>
        <div class="input">
...
<script>
  ...
  export default {
    ...
    validations: {
      //Validators need the same name as the parameter they validate
      ...
      confirmPassword: {
        // sameAs: sameAs(`password`) Add parameter as input
        //Add function as input
        sameAs: sameAs(vm => { return vm.password })
      },
      hobbyInputs: {
        required,
        minLen: minLength(2), //min length is not invalid if hobbies are not required
        $each: {
          value: {
            required,
            minLen: minLength(5)
